  ...
  const bolder = (props) =>{
   ...
   <div className="card mb-5 mb-lg-0">
     <div className="card-body">
       ...
       <hr/>
       <ul className="fa-ul">
         {/* loop for display table data and  tick and cross mark, bold  */}
         {
         props.data.features.map((ele)=><li className={`${classchecked(ele.value)} ${bold(ele.isBold)} ${bolder(ele.isBolder)}`} ><span className="fa-li">{tick(ele.value)}</span><strong>{ele.name1}</strong> {ele.name}</li>)
         }
       </ul>
